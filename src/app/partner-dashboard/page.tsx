...
interface PartnerHealthAnalytics {
...
interface PartnerSymptomData {
    name: string;
    severity: string;
    date: string;
}

interface PartnerMoodData {
    moodTypes: string[];
    intensity: string;
    date: string;
}
...
function PartnerDashboardInner() {
    ...
    const [analytics, setAnalytics] = useState<PartnerHealthAnalytics | null>(null);
    const [recentSymptoms, setRecentSymptoms] = useState<PartnerSymptomData[]>([]);
    const [recentMoods, setRecentMoods] = useState<PartnerMoodData[]>([]);
    ...
    async function loadPartnerData() {
        try {
            console.log('[partner-dashboard] Starting loadPartnerData');
            const profile = await getProfile();
            console.log('[partner-dashboard] Partner profile loaded:', profile);
            setUser(profile);

            // If partner is already connected, fetch their data
            if (profile.partnerOf) {
                console.log('[partner-dashboard] Fetching partner data for ID:', profile.partnerOf);
                ...
                if (dashboardRes.ok && dashboardRes.body?.user) {
                    console.log('[partner-dashboard] Partner data received:', dashboardRes.body.user);
                    ...
                    if (cycleRes.ok && Array.isArray(cycleRes.body) && cycleRes.body.length > 0) {
                        ...
                        const processedCycleData: any = {
                            averageCycleLength: mostRecentCycle.cycleLength,
                            lastPeriodDate: mostRecentCycle.startDate
                        };
                        console.log('[partner-dashboard] Cycle data received:', processedCycleData);
                        setCycleData(processedCycleData);
                    } else {
                        console.log('[partner-dashboard] No cycle data available');
                        setCycleData(null);
                    }

                    // Fetch analytics data
                    const analyticsRes = (await apiFetch(`/api/period-tracker/analytics?userId=${profile.partnerOf}`)) as {
                        ok: boolean;
                        status: number;
                        body?: any;
                    };

                    console.log('[partner-dashboard] Analytics response full:', analyticsRes);

                    if (analyticsRes.ok && analyticsRes.body) {
                        // Handle both response formats
                        const analyticsData = analyticsRes.body.data || analyticsRes.body;
                        console.log('[partner-dashboard] Analytics data to use:', analyticsData);
                        setAnalytics(analyticsData);
                    } else {
                        console.log('[partner-dashboard] No analytics data available, status:', analyticsRes.status);
                    }

                    // Fetch period tracker history for recent symptoms and moods
                    const trackerHistoryRes = (await apiFetch(`/api/period-tracker/history?userId=${profile.partnerOf}&limit=5`)) as {
                        ok: boolean;
                        status: number;
                        body?: any;
                    };

                    console.log('[partner-dashboard] Tracker history response:', trackerHistoryRes);
                    ...
                    if (trackers.length > 0) {
                        console.log('[partner-dashboard] Processing', trackers.length, 'trackers');

                        // Extract recent symptoms
                        const symptoms: PartnerSymptomData[] = [];
                        trackers.forEach((tracker: any) => {
                            console.log('[partner-dashboard] Tracker symptomTracking:', tracker.symptomTracking);
                        ...
                        console.log('[partner-dashboard] Extracted symptoms:', symptoms);
                        setRecentSymptoms(symptoms.slice(0, 5));

                        // Extract recent moods
                        const moods: PartnerMoodData[] = [];
                        trackers.forEach((tracker: any) => {
                            console.log('[partner-dashboard] Tracker moodTracking:', tracker.moodTracking);
                        ...
                        console.log('[partner-dashboard] Extracted moods:', moods);
                        setRecentMoods(moods.slice(0, 5));
                    } else {
                        console.log('[partner-dashboard] No tracker history available');
                    }
                } else {
                    setError("Could not load partner data");
                }
            } else {
                console.log('[partner-dashboard] No partnerOf found in profile or profile is null');
                setPartnerData(null);
                setCycleData(null);
                setAnalytics(null);
                setRecentSymptoms([]);
